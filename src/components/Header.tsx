      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-2">
            <button
              onClick={() => onRoleChange(currentRole === 'super_admin' ? 'passenger' : 'super_admin')}
            ...
            {isCompanyUser && (
              <button
                onClick={() => onRoleChange(currentRole === 'company_admin' ? 'passenger' : 'company_admin')}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-md border ${
                  currentRole === 'company_admin'
                    ? 'bg-blue-600 text-white border-blue-400 shadow-blue-600/30 ring-2 ring-blue-400/50'
                    : 'bg-blue-950/40 hover:bg-blue-900/60 text-blue-200 border-blue-500/40'
                }`}
              >
        ...
        <div className="flex md:hidden items-center justify-around pb-3 pt-2 border-t border-slate-200 dark:border-slate-800/80 text-xs font-medium">
          <button
            ...
            className={`px-2.5 py-1.5 rounded-lg flex items-center gap-1 ${currentRole === 'passenger' ? 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 font-bold border border-emerald-500/30' : 'text-slate-600 dark:text-slate-400'}`}
          >
            <Bus className="w-3.5 h-3.5" />
            <span>{isAr ? 'الرئيسية' : 'Voyages'}</span>
          </button>

          <button
            onClick={() => onRoleChange(currentRole === 'super_admin' ? 'passenger' : 'super_admin')}
            className={`px-2.5 py-1.5 rounded-lg flex items-center gap-1 ${currentRole === 'super_admin' ? 'bg-purple-500/20 text-purple-400 font-bold border border-purple-500/30' : 'text-purple-400 dark:text-purple-300'}`}
          >
          ...
          <button
            onClick={onOpenMyBookings}
            className="px-2.5 py-1.5 rounded-lg text-slate-700 dark:text-slate-300 flex items-center gap-1"
          >
            <Ticket className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
            <span>{isAr ? 'تذاكري' : 'Billets'}</span>
          </button>
          ...
          >
            ...
            <span>{isAr ? 'الشريك' : 'Partenaire'}</span>
          </button>
        </div>
